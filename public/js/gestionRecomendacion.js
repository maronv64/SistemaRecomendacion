...
function ingresarRecomendaciones(){ 
    ...
    var FrmData = {
        ...
        subtema_id: $('#idRecomendacionesS').val(),
    ...
    $.ajax({
        ...
        {
            mostrarRecomendaciones($('#idRecomendacionesS').val());     
...
function mostrarRecomendaciones(id){
    $.get('FiltrarRecomendaciones/'+id, function (data) {
...
function eliminarRecomendaciones(id){
    ...
    $.ajax({
        ...
        success: function (data)   // Una función a ser llamada si la solicitud tiene éxito
        {   
            mostrarRecomendaciones($('#idRecomendacionesS').val()); // carga los datos en la tabla                       
        }
    });
}
...
function cargartablaRecomendaciones(data){
    ...
    $("#tablaRecomendaciones").append(
        ...
        <td class='row'><button type='button' class='btn btn-info' data-toggle='modal' onClick='mostrar_recomendaciones_usuarios("+data.id+")'><i class='fa fa-building-o'></i></button></td>\
        <td class='row'><button type='button' class='btn btn-warning' data-toggle='modal' data-target='#actualizarRecomendacionesmodal'onClick='prepararactualizarRecomendaciones("+data.id+")'><i class='fa fa-edit'></i></button></td>\
...
function cargartablaRecomendacionesUsuarios(data){
    //debugger
      $("#tabla_RecomendacionesUsuarios").append(
          "<tr id='fila_cod"+"'>\
          <td>"+ data.usuarios_v2.cedula+"</td>\
          <td>"+ data.usuarios_v2.name+"</td>\
          <td>"+ data.usuarios_v2.apellidos+"</td>\
