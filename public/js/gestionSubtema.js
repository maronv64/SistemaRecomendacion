...
function mostrarSubtema(id){
    $.get('datosSubtemas/'+id, function (data) {
        $("#tablasubtema").html("");
...
function eliminarSubtema(id){
    ...
    $.ajax({
        ...
        success: function (data)   // Una función a ser llamada si la solicitud tiene éxito
        {   
            mostrarSubtema($('#idns').val());  // carga los datos en la tabla                       
        }
    });
}
...
function cargartablaSubtema(data){
    ...
    $("#tablasubtema").append(
        "<tr id='fila_cod"+"'>\
         ...
         <td class='row'><button type='button' class='btn btn-info' data-toggle='modal' data-target='#AgregarRecomendacionesmodal'onClick='Mostrar_Subtema_Recomendacion("+data.id+")'><i class= 'fa fa-	fa fa-folder-open-o'></i></button></td>\
         <td class='row'><button type='button' class='btn btn-defaul' id='btn-confirm' onClick='Mostrar_Estado_Subtema("+data.id+")'><i class='fa fa-exchange'></i></button></td>\
...
////////////////////////////////////////////////////////////////////7
function Mostrar_Subtema_Recomendacion(id){
      $('#idRecomendacionesS').val(id);
      mostrarRecomendaciones(id);
